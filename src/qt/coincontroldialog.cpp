...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = SnorcoinUnits::SNC;
    ...
    l2->setText(SnorcoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(SnorcoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(SnorcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));        // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText((fLowOutput ? (fDust ? tr("Dust") : tr("yes")) : tr("no"))); // Low Output / Dust
    l8->setText(SnorcoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change

    // turn labels "red"
    l5->setStyleSheet((nBytes >= 1000) ? "color:red;" : "");                // Bytes >= 1000
    l6->setStyleSheet((!AllowFree(dPriority)) ? "color:red;" : "");         // Priority < "medium"
    l7->setStyleSheet((fLowOutput) ? "color:red;" : "");                    // Low Output = "yes"
    l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : ""); // Change < 0.01SNC

    // tool tips
    QString toolTip1 = tr("This label turns red, if the transaction size is greater than 1000 bytes.") + "<br /><br />";
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(SnorcoinUnits::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee)) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red, if the priority is smaller than \"medium\"") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(SnorcoinUnits::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee));

    QString toolTip3 = tr("This label turns red, if any recipient receives an amount smaller than %1.").arg(SnorcoinUnits::formatWithUnit(nDisplayUnit, CENT)) + "<br /><br />";
    toolTip3 += tr("This means a fee of at least %1 is required.").arg(SnorcoinUnits::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee)) + "<br /><br />";
    toolTip3 += tr("Amounts below 0.546 times the minimum relay fee are shown as dust.");

    QString toolTip4 = tr("This label turns red, if the change is smaller than %1.").arg(SnorcoinUnits::formatWithUnit(nDisplayUnit, CENT)) + "<br /><br />";
    toolTip4 += tr("This means a fee of at least %1 is required.").arg(SnorcoinUnits::formatWithUnit(nDisplayUnit, CTransaction::nMinTxFee));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                sAddress = CSnorcoinAddress(outputAddress).ToString().c_str();

                // if listMode or change => show snorcoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, SnorcoinUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, SnorcoinUnits::format(nDisplayUnit, nSum));
